...
module.exports = {
    ...
    try{
      let search = await bing(context, args.query, false)
      search = search.response
     
      if(search.body.status === 2) return editOrReply(context, createEmbed("error", context, search.body.message))
      if(search.body.status === 5) return editOrReply(context, createEmbed("nsfw", context, "This search is primarily adult-oriented and can only be ran in Age Restricted channels."))
