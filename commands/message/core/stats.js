  ...
  run: async (context) => {
    ...
    try{
      if(context.manager){
        ...
        const display = [
          `${iconPill("home",      "Servers      ")} ${highlight(` ${formatNumber(formatted.guilds, 1)} `)}`
        ] 
        display.push(`${iconPill("user",      "Installations")} ${highlight(` ${formatNumber(context.client.application.approximateUserInstallCount, 1)} `)}`)
