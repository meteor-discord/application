...
module.exports = {
  ...
  run: async context => {
    ...
    if (!context.message.messageReference)
      return editOrReply(context, createEmbed('warning', context, 'You need to reply to a voice message.'));
    try {
      let msg;
      try {
      ...
      if (!msg.attachments.first())
        return editOrReply(context, createEmbed('warning', context, 'No voice message found.'));
      if (!msg.attachments.first().url.split('?')[0].endsWith('voice-message.ogg'))
        return editOrReply(context, createEmbed('warning', context, 'No voice message found.'));
