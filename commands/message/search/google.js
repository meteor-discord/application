...
function createSearchResultPage(context, result, doodle) {
  ...
  switch (result.type) {
    ...
    case SEARCH_CARD_TYPES.ENTITY:
      ...
      if (result.card.images.image)
        res.image = {
          url: result.card.images.image,
        };

      if (result.card.facts)
        res.fields = result.card.facts.map(f => {
          return {
            name: f.label,
            value: f.value,
            inline: true,
...
module.exports = {
  ...
  metadata: {
    ...
    examples: ['g Eurasian Small Clawed Otter', 'g 5 euro to dollar', 'g time in berlin', 'g $goog'],
    category: 'search',
    usage: 'google <query>',
    slashCommand: 'google',
  },
