...
import { createLinkButtons, formatArrayWithEllipsis, formatRelativeTimestamp, getStatusIcon } from '~/lib/utils'
...
export default class Info extends Command {
  ...
  private async application(interaction: ChatInputCommandInteraction, $: I18nFunction): Promise<InteractionResponse<boolean>> {
    ...
    const row = createLinkButtons([
      {
        label: $('modules.info.app.buttons.icon'),
        url: `https://cdn.discordapp.com/avatars/${applicationId}/${data.icon}?size=1024`,
      },
      {
        label: $('modules.info.app.buttons.invite'),
        url: `https://discord.com/api/oauth2/authorize?client_id=${applicationId}&scope=bot`,
      },
      {
