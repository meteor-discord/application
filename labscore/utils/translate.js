...
module.exports.getDictionaryFromAny = function (prompt) {
  ...
  for(const i of Object.keys(DICTIONARY_LANGUAGES)) if(!languages.includes(i) && (DICTIONARY_LANGUAGES[i].toLowerCase() == prompt.toLowerCase() || DICTIONARY_LANGUAGES[i].toLowerCase().includes(prompt.toLowerCase()))) languages.push(i)
  return languages;
};
